...
public class AgentRuntime {
    ...
    public AgentResponse invoke(AgentRequest request) {
        ...
        if (gate == HitlGuard.Decision.PENDING_APPROVAL) {
            String auditId = writeAudit(tenantId, traceId, def.agentId(), "AGENT_GATE",
                    def.riskLevel(), "PENDING_APPROVAL",
        ...
        try {
            response = agent.handle(new AgentRequest(
                    def.agentId(), traceId, tenantId, request.actorId(), request.input()));
        } catch (RuntimeException ex) {
            String auditId = writeAudit(tenantId, traceId, def.agentId(), "AGENT_ERROR",
                    def.riskLevel(), "DENIED",
                    "Agent threw exception: " + ex.getClass().getSimpleName() + ": " + ex.getMessage(),
                    String.valueOf(request.input()), null,
                    System.currentTimeMillis() - start);
            return AgentResponse.denied(ex.getMessage(), auditId);
        }

        String auditId = writeAudit(tenantId, traceId, def.agentId(), "AGENT_RUN",
                def.riskLevel(), response.status(),
                response.reasoningSummary(),
                String.valueOf(request.input()), String.valueOf(response.decision()),
                System.currentTimeMillis() - start);

        if (response.auditLogId() == null) {
            return new AgentResponse(response.status(), response.decision(),
                    response.reasoningSummary(), auditId);
        }
        return response;
    }

    private String writeAudit(String tenantId, String traceId, String agentId,
                              String action, String riskLevel, String decision,
                              String reasoning, String input, String output, long latency) {
