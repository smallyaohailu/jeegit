...
/**
 * Demo seed data — makes sure the reference flows in the README work on a fresh install. Production
 * deployments should use externalized migrations (Flyway / Liquibase) rather than this runner.
 */
@Component
public class DemoDataSeed implements CommandLineRunner {
    ...
    return orgService.listForCurrentTenant().stream()
        ...
        .orElseGet(
            () -> {
              ...
              orgService.create(
                  new Org("GEN", "General Intake Window", Org.Type.DEPARTMENT), root.getId());
              orgService.create(new Org("TAX", "Tax Bureau", Org.Type.DEPARTMENT), root.getId());
              orgService.create(
                  new Org("SOCIAL", "Social Security Bureau", Org.Type.DEPARTMENT), root.getId());
              return root;
            });
  }
  ...
  private void ensureDefaultRoles() {
    ...
    if (roleRepository.findByTenantIdAndCode(tenantId, "ROLE_DISPATCHER").isEmpty()) {
      Role dispatcher =
          new Role("ROLE_DISPATCHER", "Intake Dispatcher", DataScope.COMPANY_AND_CHILD);
  ...
  private void ensureDispatchRuleDict() {
    ensureType(
        "MATTER_DISPATCH_RULE",
        "Matter Dispatch Rules",
