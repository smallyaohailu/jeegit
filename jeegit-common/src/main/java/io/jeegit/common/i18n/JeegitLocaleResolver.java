...
/**
 * Locale resolver that combines, in order of precedence:
 *   1. A {@code jeegit_locale} cookie written by the UI (or by
 *      {@link org.springframework.web.servlet.i18n.LocaleChangeInterceptor}).
 *   2. The {@code Accept-Language} request header.
 *   3. {@link SupportedLocales#DEFAULT}.
 *
 * Every resolved locale is folded back onto the supported list by
 * {@link SupportedLocales#negotiate(Locale)} so callers always get one of
 * the twelve supported locales.
 */
public class JeegitLocaleResolver implements LocaleResolver {
    ...
    public void setLocale(HttpServletRequest request, HttpServletResponse response, Locale locale) {
        Locale effective = locale == null ? SupportedLocales.DEFAULT : SupportedLocales.negotiate(locale);
